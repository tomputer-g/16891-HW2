...
def KRCBSSymmetricSplittingConstraints(collision: KRCBSEdgeCollision | KRCBSVertexCollision, k: int):
    if type(collision) is KRCBSVertexCollision:
        constraints_agent1 = []
        constraints_agent2 = []
        for t in range(max(0, collision.timestep2-k), collision.timestep2+k+1):
            constraints_agent1.append(KRCBSConstraint(agent=collision.a1, loc=collision.loc, timestep=t).to_dict())
...
class KRCBSSolver(object):
    ...
    def find_solution(self):
        ...
        while len(self.open_list) > 0:
            ...
            constraints: List[List[Dict[str, Any]]] = KRCBSSymmetricSplittingConstraints(collision=collision, k=self.k)
